...
const getPortfolioData = async (req, res) => {
    try {
        ...
        for (const transaction of transactions) {
            const { stockName, quantity, pricePerShare, transactionType } = transaction;
            ...
            if (transactionType === 'Buy') {
                ...
                portfolio[stockName].avgPurchasePrice = parseFloat((portfolio[stockName].totalInvested / portfolio[stockName].totalQuantity).toFixed(2));
            } else if (transactionType === 'Sell') {
                // Ensure quantity doesn't go negative
                if (portfolio[stockName].totalQuantity >= quantity) {
                    portfolio[stockName].totalQuantity -= quantity;
                    portfolio[stockName].totalInvested -= portfolio[stockName].avgPurchasePrice * quantity;
                } else {
                    console.warn(`Warning: Sell quantity exceeds available quantity for ${stockName}`);
        ...
        for (const stockName in portfolio) {
            ...
            portfolio[stockName].currentStockPrice = parseFloat((currentStockPrice || 0).toFixed(2));
...
async function getCurrentStockPrice(stockName) {
    try {
        ...
        console.error(`Error fetching stock price for ${stockName}:`, error.message);
